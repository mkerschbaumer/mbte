...
# generate a tbl_mbte with the data generated by gen_raw_dataset()
# NOTE: the attributes "time" and "value" are set accordingly
gen_raw_tbl_mbte <- function(long = TRUE, mv_prefix = "mv") {
  tbl <- gen_raw_dataset(long, mv_prefix)
  new_tbl_mbte(tbl, time = time, value = value)
}

# generate a grouped tbl_mbte with the dataset generated by gen_raw_dataset()
# NOTE: the attributes "time" and "value" are set accordingly
gen_raw_gr_tbl_mbte <- function(long = TRUE, mv_prefix = "mv") {
  mv_prefix_sym <- rlang::sym(mv_prefix)
  tbl <- gen_raw_dataset(long, mv_prefix) %>% dplyr::group_by(!!mv_prefix_sym)
  new_tbl_mbte(tbl, time = time, value = value)
}
